...
def _split_numeric_feature_types(df: pd.DataFrame, numeric_cols: list[str], discrete_threshold: int) -> tuple[list[str], list[str]]:
...
def apply_feature_engineering(
    ...
    - PolynomialFeatures on numeric columns
    ...
    # Split numeric columns so coded-discrete features remain interpretable.
    ...
    passthrough_force = {
        str(c).strip() for c in (force_passthrough_cols or []) if str(c).strip()
    }
    ...
    # Missing value indicators are only added for continuous columns used in poly features.
    ...
    # Polynomial features are generated only from continuous numeric columns.
    poly_names: list[str] = []
    ...
    if save_csv:
        ...
        # Use UTF-8-SIG to ensure Excel displays Turkish characters correctly
        df_engineered.to_csv(fe_path, index=False, encoding="utf-8-sig")

    # Numeric outputs include poly columns, continuous missing flags and preserved discrete-coded numerics.
